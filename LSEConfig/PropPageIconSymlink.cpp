...
BOOL PropPageIconSymlink::OnInitDialog()
{
  ...
  gLSESettings.ChangegFlags(eSymboliclinkOverlay, &m_Overlay, true);
  ...
  int Prio = 0;
  gLSESettings.GetValue(LSE_REGISTRY_SYMBOLICLINK_OVERLAY_PRIO, &Prio);
  ...
  OverlayPath[0] = 0x00;
  gLSESettings.ChangeValue(LSE_REGISTRY_SYMBOLICLINK_ICON, OverlayPath, MAX_PATH);
  if (OverlayPath[0])
...
int PropPageIconSymlink::FetchOverlayPrio()
{
  ...
  if(EOF != swscanf_s(buf, L"%d", &Prio))
    gLSESettings.SetValue(LSE_REGISTRY_SYMBOLICLINK_OVERLAY_PRIO, Prio);
...
int PropPageIconSymlink::FetchOverlayPath(bool aCommit)
{
  ...
  {
    if (aCommit)
      gLSESettings.ChangeValue(LSE_REGISTRY_SYMBOLICLINK_ICON, buf, MAX_PATH);
    LoadIconPreview(buf);
  }
...
{
  gLSESettings.ChangegFlags(eSymboliclinkOverlay, NULL, m_Overlay);

  FetchOverlayPrio();
  if (m_OverlayPathCustom)
    FetchOverlayPath(true);
  else
    gLSESettings.DeleteValue(LSE_REGISTRY_SYMBOLICLINK_ICON);
}
