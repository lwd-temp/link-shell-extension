...
@if [%2] NEQ [smartrename] ( 
  ...
  REM
  REM SM02_T03 Move trees that start with a reparse point
  REM
  @%LN% %ABS_REL% %OPTION% %TESTROOTDST% %TESTROOTSRC% > nul
  %LN% %REPARSEOPT% %TESTROOTSRC%\Base %TESTROOTSRC%\BaseReparse > nul
  %LN% %ABS_REL% %OPTION% %TESTROOTSRC%\BaseReparse %TESTROOTDST%  > sortout
  @echo off
  set ERRLEV=%errorlevel%
  sort sortout
  @echo on
  ...
  @%RD% %TESTROOTSRC%
  @%RD% %TESTROOTDST% 
  %MKDIR% %TESTROOT%\Folder > nul
  %MKDIR% %TESTROOT%\NewFolder  > nul
  %MKDIR% %TESTROOT%\Folder\SubFolder > nul
  %LN% %REPARSEOPT% %TESTROOT%\Folder\SubFolder %TESTROOT%\Folder\SymlinkSubFolder > nul
  echo on
  pause
  %LN% %ABS_REL% %OPTION% %TESTROOT%\Folder %TESTROOT%\NewFolder\Folder
  pause
