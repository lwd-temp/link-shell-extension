...
// #define LANGDLL_DEBUG 1
  ...
  switch (uMsg) 
  {
  	case WM_INITDIALOG:
    {
      // Find out which language is installed on this system
...
//      strcpy(InstalledLanguage, "2031");
      ...
      {
#if defined LANGDLL_DEBUG
      {
        char s[256];
        sprintf(s, "LSEINSTALL: langs[i].id %s", langs[i].id);
        OutputDebugString(s);
      }
#endif
        if ( !strcmp(langs[i].id, InstalledLanguage) )
        {
          InstalledLanguagePos = i;
          break;
        }
      }
      if (InstalledLanguagePos > 0)
        selected_language = langs[InstalledLanguagePos].name;

#if defined LANGDLL_DEBUG
      {
        char s[256];
        sprintf(s, "LSEINSTALL: InstalledLanguage %s, InstalledLanguagePos %d, selected_language %s", InstalledLanguage, InstalledLanguagePos, selected_language);
        OutputDebugString(s);
      }
#endif
      ...
      for (i = langs_num - 1; i >= 0; i--) 
      {
        ...
        if (!lstrcmp(langs[i].id, getuservariable(INST_LANG)) && !selected_language) {
